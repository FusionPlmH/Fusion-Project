...
public  class LeoLockScreenTile extends QSTileImpl<QSTile.BooleanState> {

     Icon mCurrentIcon;
    public int mIconId;
    public int mStringId;
    public final int  mLEO_LOCK_SCREEN_MODE=100;
    public LeoLockScreenTile(QSHost qSHost) {
        super(qSHost);
        setIds();
    }

    private void setIds() {
        this.mIconId = getLeoResource("drawable/quick_panel_icon_lockscreen");
        this.mCurrentIcon = ResourceIcon.get(this.mIconId);
        this.mStringId = getLeoResource("string/quick_settings_lockscreen_label");
    }
    ...
    protected void handleClick() {
        if (getLeoOS().equals(getOnema())) {//判断ROM 版本
            if (getCode() .equals(getOCodeN())) {//判断ROM 源码版本
                    ...
                    LeoGlobalActiong(mContext,mLEO_LOCK_SCREEN_MODE,null);
