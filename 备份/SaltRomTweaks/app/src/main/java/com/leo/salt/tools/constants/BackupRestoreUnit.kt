...
class BackupRestoreUnit(var context: Context) {
    ...
    internal inner class FlashZIPThread(var path: String): Thread() {
        override fun run() {
            try {
              ...
              doCmdSync( "mkdir /cache/recovery")
            doCmdSync("cp  $path /cache/recovery/leoupdate.zip")
            doCmdSync( "${Consts.SDcommandUrl}")
            doCmdSync("sleep 5")
            doCmdSync(URL)
            } catch (e: IOException) {
                e.printStackTrace()
    ...
    fun InstallFlashKernel() {
        FlashKernel().start()
    }
    internal inner class FlashKernel(): Thread() {
        override fun run() {
            try {
                ShowProgressBar()
                doCmdSync(Consts.MountSystemRW)
                doCmdSync( "mkdir /cache/recovery")
                doCmdSync("cp  $KernelInstalluRL /cache/recovery/leoupdate.zip")
                doCmdSync(Consts.SDcommandUrl)
                doCmdSync("sleep 5")
    ...
    internal inner class FlashZIP(): Thread() {
        override fun run() {
            try {
                doCmdSync("mount -o rw,remount /data")
                doCmdSync( "mount -o rw,remount /system")
                doCmdSync( "mkdir /cache/recovery")
