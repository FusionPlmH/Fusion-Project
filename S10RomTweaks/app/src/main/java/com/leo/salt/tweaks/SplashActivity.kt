...
class SplashActivity : Activity() {
    ...
    private class CheckFileWirte(context: SplashActivity) : Runnable {
        private var context: WeakReference<SplashActivity> = WeakReference(context)
        override fun run() {

            context.get()!!.hasRoot = true
           context.get()!!.checkFileWrite()
        }

    }

    private fun checkFileWrite() {
        KeepShellPublic.doCmdSync("pm grant ${this.packageName} android.permission.WRITE_SECURE_SETTINGS")
        KeepShellPublic.doCmdSync("pm grant ${this.packageName} android.permission.WRITE_SETTINGS")
    }
