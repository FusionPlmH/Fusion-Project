...
import static com.leo.salt.tweaks.view.widget.DialogView.linkCountDialog;
import static com.leo.salt.utils.BatteryUtil.*;;
import static com.leo.salt.utils.CpuInfo.*;
...
public class ExtraInfoFragment extends Fragment implements View.OnClickListener {
   ...
   private TextView mCPU;
     ...
     public void getData(){
        ...
        String strbbt=bbt.substring(0, 4);
             mMake.setText(getStringIdentifier(mContext,"device_inf_vendor")+mLine+toUpperCaseFirstOne(getLeoBuild("ro.product.manufacturer"))
                     +mLine2+getStringIdentifier(mContext,"device_inf_origin")+mLine+producing_area(getLeoSafetyID())
                     +mLine2+getStringIdentifier(mContext,"device_inf_of_sale_csc")+mLine+getLeoBuild("ro.boot.carrierid")+mLine2+
    ...
    public void onClick(View v) {
        linkCountDialog(mContext,getString(R.string.link_count));
    }
}
