...
public class HighlightablePreferenceGroupAdapter extends PreferenceGroupAdapter {
     ...
     */
    /*
    public static void adjustInitialExpandedChildCount(BaseSettingsFragment host) {
        if (host == null) {
            return;
        }
        ...
        if (arguments != null) {
            final String highlightKey = arguments.getString(EXTRA_PREFERENCE_KEY);
    ...
    */
    ...
    public void requestHighlight(View root, final RecyclerView recyclerView) {
        if (mHighlightRequested || recyclerView == null || TextUtils.isEmpty(mHighlightKey)) {
            return;
        }
        root.postDelayed(() -> {
            final int position = HighlightablePreferenceGroupAdapter.this.getPreferenceAdapterPosition(mHighlightKey);
            if (position < 0) {
                return;
            }
            ...
            HighlightablePreferenceGroupAdapter.this.notifyItemChanged(position);
