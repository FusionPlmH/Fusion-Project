...
public  class NotificationUtils {
    ...
    public static void  showRebootNotification(Context context,String information) {
       ...
       Intent showIntent = new Intent(Constants.REBOOT_DEVICE);
        ...
        NotificationCompat.Action action = new NotificationCompat.Action.Builder(R.drawable.ic_reboot,"重启",contentIntent).build();
        // 创建notification，使用设置优先级的方式创建悬浮通知，则会自动消失
        NotificationCompat.Builder builder = new NotificationCompat.Builder(context)
                .setSmallIcon(R.drawable.ic_stat_name)
                .setContentTitle(context.getResources().getString(R.string.reboot_required_dialog_title))
                .setContentText(information)
               // .setColor(Color.CYAN)
                .setLargeIcon(BitmapFactory.decodeResource( context.getResources(), R.drawable.ic_reboot))
    ...
    public static void  showOtaNotification(Context context) {

        Intent showIntent = new Intent(Constants.ROM_OTA);
        showIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        PendingIntent contentIntent = PendingIntent.getBroadcast(context, 0,
                showIntent,
                PendingIntent.FLAG_UPDATE_CURRENT);
        NotificationCompat.Action action = new NotificationCompat.Action.Builder(R.drawable.ic_reboot,"更新",contentIntent).build();
        // 创建notification，使用设置优先级的方式创建悬浮通知，则会自动消失
        NotificationCompat.Builder builder = new NotificationCompat.Builder(context)
                .setSmallIcon(R.drawable.ic_stat_name)
                .setContentTitle(context.getResources().getString(R.string.reboot_required_dialog_title))
                .setContentText("5秒后自动开始自动开始更新,点击立马更新")
                // .setColor(Color.CYAN)
