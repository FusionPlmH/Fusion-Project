...
class DialogSingleAppOptions(context: Context, var app: Appinfo, handler: Handler) : DialogAppOptions(context, arrayListOf(app), handler) {
    ...
    private fun moveToSystemMagisk() {
        ...
        if (result) {
            Toast.makeText(context, getApksStringInt(R.string.apks_magisk_uid), Toast.LENGTH_LONG).show()
        } else {
            Toast.makeText(context, getApksStringInt(R.string.apks_magisk_nox), Toast.LENGTH_LONG).show()
        }
    }
