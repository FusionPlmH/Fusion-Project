...
import com.salt.script.Consts.BackUpDir
...
class AppListHelper(context: Context) {
    ...
    fun getTags(applicationInfo: ApplicationInfo): String {
        ...
        try {
            ...
            val absPath =AbsBackUpDir + packageName + ".apk"
            ...
            } else if (File(BackUpDir + packageName + ".tar.gz").exists()) {
                stateTags.append("☆")
            }
        } catch (ex: Exception) {
        }
        ...
        for (i in files.indices) {
            ...
            try {
                ...
                if (packageInfo != null) {
                    ...
                    if (applicationInfo.packageName == "com.salt.config.tweaks")
                        continue
