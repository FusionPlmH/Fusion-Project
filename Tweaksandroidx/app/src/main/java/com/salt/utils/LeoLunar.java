...
import com.salt.frame.LeadFrame;
...
import static com.salt.frame.LeadFrame.getStringLeoArray;
...
public class LeoLunar{
    ...
    public static String[] ConstellationArray(Context context) {
        return getStringLeoArray( context,rt100foleo("Y29uc3RlbGxhdGlvbl8xMlRpbWU="));
    }
    public static final int[] ConstellationEdgeDay = { 20, 19, 21, 21, 21, 22,
            23, 23, 23, 23, 22, 22 };

    public static String ConstellationString(Context context,Calendar time) {
       int month = time.get(Calendar.MONTH);
        ...
        if (month >= 0) {
            return ConstellationArray(context)[month];
        }
        // default to return 魔羯
        return ConstellationArray(context)[11];
    }
    public static String DATEView(Context context,String STR){

        return LeadFrame.getStringIdentifier(context,STR);
    }

    public  String getToday(Context ctx){

        android.icu.util.Calendar instance = android.icu.util.Calendar.getInstance();
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat(DATEView(ctx,"constellation_date_format"));
        ...
        int yearOfNumber = cd.get(java.util.Calendar.DAY_OF_YEAR);//获得当天是一年中的第几天
        int week = cd.get(cd.WEEK_OF_YEAR);
        String str;

        if(isChineseLanguage()==true){
            str=simpleDateFormat.format(date)+" "+ConstellationString(ctx,instance)+"\n"+"农历:"+toLeoLunarChineseLunar(true,true,0,5)+"\n"+simpleDateFormat2.format(date)+"年中的第"+" "+week+" 周"+" "+"第"+yearOfNumber+"天";
        }else {
