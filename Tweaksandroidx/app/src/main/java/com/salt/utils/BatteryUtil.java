...
import static com.salt.utils.ExtraInfo.ReadFile;
...
public class BatteryUtil {
    ...
    static float getBatteryTemperature() {
        return ((float) SaltApp.getContext().registerReceiver(null, new IntentFilter("android.intent.action.BATTERY_CHANGED")).getIntExtra("temperature", 0)) / 10.0f;
    }
  ...
  public  static  enum TemperatureType {
   ...
   static float getCPUTemperature() {
    ...
    public static String getVoltage() {
        String result = SaltApp.getContext().getString(R.string.get_error) ;
        try {
   ...
   public static String getUsableindex (Context context) {
    ...
    public static String getDesignCapacity() {
        ...
        return getBatteryCapacity(SaltApp.getContext());
    }
  ...
  public   static float cgvvdf() {

        return Float.valueOf(ReadFile("/sys/class/power_supply/battery/capacity")).floatValue();
    }
