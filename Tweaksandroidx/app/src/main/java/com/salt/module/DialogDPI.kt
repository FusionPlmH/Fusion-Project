...
import com.salt.setting.SaltSetting.GLOBAL_SPF
import com.salt.widget.DialogView.showSnackn
...
class DialogDPI(var context: Context) {
    ...
    fun modifyDPI(display: Display, context: Context) {
        ...
        dialog.findViewById<TextView>(R.id.dialog_dpi_1080).setOnClickListener({
            ...
            showSnackn(dialog,"FHD+(2220×1080)");
        ...
        dialog.findViewById<TextView>(R.id.dialog_dpi_2k).setOnClickListener({
            ...
            heightInput.setText(getHeightScaleValue(width).toString())
            dpiInput.setText(getDpiScaleValue(width).toString())
            showSnackn(dialog,"WQHD+(2960×1440)");

        })



        val dialogInstance = AlertDialog.Builder(context).setTitle(getStringIdentifier(context, "grid_dpi")).setView(dialog).setNegativeButton(android.R.string.cancel,{ dialog, _ -> dialog.dismiss()  }).setPositiveButton(android.R.string.yes, { _, _  ->
