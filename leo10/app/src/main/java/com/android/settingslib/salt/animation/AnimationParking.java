package com.android.settingslib.salt.animation;
...
public class AnimationParking {
	...
	private static Runnable runnable4Parking = () -> parking();
	...
	private static Runnable runnable4Shrink = () -> shrinking();
	private static Runnable runnable4Turning = () -> turning();
	...
	MeterBase.MeterMap.get(MeterBack.NAME).setVisibility(View.INVISIBLE);
	MeterBase.MeterMap.get(MeterFlashlight.NAME).setVisibility(View.INVISIBLE);
	}
	...
	public static void updateAll(int x, int y) {
		...
		MeterBase.MeterMap.get(MeterHome.NAME).update(x, y);
		MeterBase.baseX = x;
		MeterBase.baseY = y;
		if (mOriginSide) {
			updateAllLeft(x, y);
	...
	private static void updateAllLeft(int x, int y) {
		...
		MeterBase.MeterMap.get(MeterBack.NAME).update(homeX, homeY);
		MeterBase.MeterMap.get(MeterFlashlight.NAME).update(menuX, menuY);
	}
