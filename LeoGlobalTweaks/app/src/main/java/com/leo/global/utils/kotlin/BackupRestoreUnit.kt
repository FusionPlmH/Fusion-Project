...
import com.leo.global.utils.kotlin.KeepShellSync.doCmdSync
...
class BackupRestoreUnit(var context: Context) {
    ...
    fun runGetCommand(packageNameToKill: String) {
        //      RootUtils.runCommand("pkill -f " + packageNameToKill);
        doCmdSync(packageNameToKill)
    }
    //隐藏进度条
    fun HideProgressBar() {
        myHandler.post {
    ...
    fun FlashBootUrl(path: String) {
        ...
        if (getLeoBuild("ro.hardware.chipname").equals("exynos8895")) {
