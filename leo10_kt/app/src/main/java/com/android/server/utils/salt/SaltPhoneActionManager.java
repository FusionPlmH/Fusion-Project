...
import android.net.AConnectivityManager;
import android.net.ConnectivityManager;
import android.net.Uri;
import android.net.wifi.AWifiManager;
...
import android.telephony.ATelephonyManage;
...
public class SaltPhoneActionManager {
	public final static String TAG = SaltPhoneActionManager.class.getSimpleName();

	public  Context mContext;
    public static  String mAppsManager="Y29tLmxlby5BQ1RJT05fQVBQUw==";
    public static  String mWIFIManager="Y29tLmxlby5BQ1RJT05fV0lGSQ==";
    public static  String mLeoManager="com.salt.config/com.salt.config.SplashActivity";
	...
	private static  String mKillWhiteList= "com.android.systemui##com.android.phone##com.salt.config##com.sec.android.app.launcher";
	public SaltPhoneActionManager(Context context){
		mContext=context;

	}
	public void startConfigAction(String str){
		try {
			Intent intent = new Intent();
			intent.setFlags(FLAG_ACTIVITY_NEW_TASK);
			intent.setAction(rt100foleo(str));
    ...
    private static final class ExpandAction {
       ...
       private static void expandNotificationsPanel() {
		...
		public  void toggleExpandedDesktop(Context context,String str) {
			...
			if (newVal.equals("com.salt.config")) {
    ...
    public void toggleMobile() {
		Context context=mContext;
		ATelephonyManage telephonyManager = (ATelephonyManage)context.getSystemService("phone");
	...
	public void toggleWifiAP() {
			...
			AWifiManager wm = (AWifiManager) context.getSystemService(Context.WIFI_SERVICE);
			final AConnectivityManager mConnectivityManager;
			mConnectivityManager = (AConnectivityManager) context.getSystemService(
					Context.CONNECTIVITY_SERVICE);
			int state = wm.getWifiApState();
			boolean enabled = false;
			switch (state) {
				case AWifiManager.WIFI_AP_STATE_ENABLING:
				case AWifiManager.WIFI_AP_STATE_ENABLED:
					enabled = false;
					break;
				case AWifiManager.WIFI_AP_STATE_DISABLING:
				case AWifiManager.WIFI_AP_STATE_DISABLED:
					enabled = true;
					break;
			}
			...
			if (enabled) {
				...
				mConnectivityManager.startTethering(
						AConnectivityManager.TETHERING_WIFI, false, callback);
			} else {
				mConnectivityManager.stopTethering(AConnectivityManager.TETHERING_WIFI);
			}
	 ...
	 class OnStartTetheringCallback extends
			AConnectivityManager.OnStartTetheringCallback {
		@Override
	...
	public  void performRestartTask( String action,String processNa) {
		if (action == null) {
			return;
		}
		...
		if (action.equals(rt100foleo("bGVvLmFjdGlvbi5TeXN0ZW1VaQ=="))) {
			try {
				AAActivityManager am =
						( AAActivityManager)mContext.getSystemService(Context.ACTIVITY_SERVICE);
			...
			((aPowerManager)mContext.getSystemService(Context.POWER_SERVICE)).shutdown(false, null, false);
			return;
		}
	}
