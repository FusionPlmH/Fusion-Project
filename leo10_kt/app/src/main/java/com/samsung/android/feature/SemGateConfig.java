...
public final class SemGateConfig {
    ...
    public static boolean isGateEnabled() {
        ...
        if ("user".equals(Build.TYPE)) {
            return valueOf.equals(DEBUG_LEVEL_LOW) ? false : SystemProperties.get(GATE_SYS_PROP_GATE_ENABLED).equals("1");
        } else {
            return !sGateEnabled ? SystemProperties.get(GATE_SYS_PROP_GATE_ENABLED).equals("1") : true;
        }
    }
    ...
    public static boolean isGateLcdtextEnabled() {
        String valueOf = String.valueOf(SystemProperties.get("ro.debug_level"));
        if ("user".equals(Build.TYPE)) {
            return valueOf.equals(DEBUG_LEVEL_LOW) ? false : SystemProperties.get(GATE_SYS_PROP_LCDTEXT_ENABLED).equals("1");
        } else {
            return !sGateLcdtextEnabled ? SystemProperties.get(GATE_SYS_PROP_LCDTEXT_ENABLED).equals("1") : true;
        }
    }
