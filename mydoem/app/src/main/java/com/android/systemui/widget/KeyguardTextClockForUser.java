...
public class KeyguardTextClockForUser extends TextClock implements SystemUIWidgetCallback {
   ...
   // private KeyguardFontEffectUtil mFontEffectUtil;
    ...
    protected void onDraw(Canvas canvas) {
        if (this.mAdaptiveColors == null || !this.mAdaptiveColorEnabled || this.mResData.timeIndex == 1) {
        ...
        Rect bound = new Rect();
        getLocalVisibleRect(bound);
        if (this.mIsVertical) {
            bound.bottom *= 2;
        }
        int shadowColor = 1711276032;
        if (isWhiteKeyguardWallpaper) {
            shadowColor = 1996488703;
        }
        ...
        paint.setShadowLayer(shadowRadius, getShadowDx(), shadowDy, shadowColor);
        paint.setShader(null);
        super.onDraw(canvas);
        float gradientLength = (float) bound.bottom;
        float[] positions = new float[]{0.0f, 1.0f};
        double angleInRadians = Math.toRadians(90.0d);
        LinearGradient shader = new LinearGradient(0.0f, 0.0f, Math.abs(((float) Math.cos(angleInRadians)) * gradientLength), Math.abs(((float) Math.sin(angleInRadians)) * gradientLength), this.mAdaptiveColors, positions, TileMode.CLAMP);
        paint.setColor(-1);
        paint.clearShadowLayer();
        paint.setShader(shader);
        if (this.mIsVertical && this.mResData.timeIndex == 2) {
            Matrix matrix = new Matrix();
            shader.getLocalMatrix(matrix);
            matrix.postTranslate(0.0f, -(gradientLength / 2.0f));
    ...
    private void updateFont() {
        ...
        Object path = null;
        if (SettingsHelper.getInstance().isOpenThemeLook()) {
            path = SettingsHelper.getInstance().getOpenThemeClockFont();
        }
