...
public class Carousal extends AbstractTransitionEffects {
    ...
    protected float getTranslationValueDeltaX(float f, float f2) {
        return ((DragView.EXTRA_VIEW_SCALE_FACTOR - f2) - (WallpaperUtils.WALLPAPER_SCREENS_SPAN * (0.1f * Math.abs(f)))) * this.mShrinkTranslateX;
    }
    ...
    protected float getTranslationValueY(float f, float f2) {
        return ((DragView.EXTRA_VIEW_SCALE_FACTOR - f2) - (WallpaperUtils.WALLPAPER_SCREENS_SPAN * (0.1f * Math.abs(f)))) * this.mShrinkTranslateY;
    }

    protected float getZoomValueX(float f, float f2) {
        return f2 - (WallpaperUtils.WALLPAPER_SCREENS_SPAN * (0.1f * Math.abs(f)));
    }
