...
public class Conveyor extends AbstractTransitionEffects {
    ...
    protected float getRotationValue(float f, float f2) {
        float f3 = ((!PageTransitionManager.mLeftMove || f < 0.0f) && (PageTransitionManager.mLeftMove || f >= 0.0f)) ? 0.0f : -f2;
        return this.mEndPage ? ((-(transition_rotation_max / WallpaperUtils.WALLPAPER_SCREENS_SPAN)) * f) / maxOverScroll() : f3;
    }
