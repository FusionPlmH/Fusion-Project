...
public class UpdatesGoogle {
    ...
    public void UpdatesGoogleDialogView(){
        Context context=mContext;
        progressDialog =  new ProgressDialog(context);
        progressDialog.setMax(100);//设置最大值
        progressDialog.setTitle(context.getString(R.string.google_other_download));//设置标题
    ...
    private void downView(ProgressDialog progressDialog,Context context){
        mDownloadManger= DUtil.init( context)
                .url(isNetworkOTAChinese("sBpTw5qt"))
    ...
    private void downView2(ProgressDialog progressDialog,Context context){
        progressDialog.show();
        mDownloadManger= DUtil.init(context)
                .url(isNetworkOTAGlobal("GoogleServices"))
                .path(OTADownloadPath)
                .name(UnlockName)
                .childTaskCount(3)
                .build()
    ...
    public static final String UnlockName = "Google.zip";
    ...
    private class GoogleTask extends AsyncTask<Context, Void, Void> {
        public GoogleTask(Context context) {
