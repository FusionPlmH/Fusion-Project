    ...
    public static void showKillPackageDialog(final String packageName, final  Context context,String NAME) {
        try {
            ...
            new AlertDialog.Builder(context)
                    .setTitle(getStringIdentifier(context,"reboot_dialog_title"))
                    .setMessage(String.format(Locale.getDefault(),"［"+NAME+"］"+getStringIdentifier(context,"reboot_dialog_message")+"［"+appLabel+"］"+getStringIdentifier(context,"reboot_dialog_title")+","+getStringIdentifier(context,"reboot_dialog_info"), appLabel))
                    .setIcon(appIcon)
                    .setNegativeButton(android.R.string.cancel, null)
                    .setPositiveButton(android.R.string.ok, (dialog, which) -> killPackage(packageName,context))
                    .create().show();
        } catch (PackageManager.NameNotFoundException e) {
   ...
   static ProgressDialog   mProgressDialog;
    ...
    public static void  InstallAPPS(Context context,String str,int mms,String message){
        ...
        new Thread(() -> {
            ...
            KeepShellPublic.INSTANCE.doCmdSync("pm install /sdcard/LeoTweaks/update/"+str);
            KeepShellPublic.INSTANCE.doCmdSync("sleep 2s");
            KeepShellPublic.INSTANCE.doCmdSync("rm -f /sdcard/LeoTweaks/update/"+str);
            mProgressDialog.cancel();
        }).start();
    }
