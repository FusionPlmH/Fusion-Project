...
public class BackupRestoreService extends IntentService {
    ...
    private static final String LOG_TAG = BackupRestoreService.class.getSimpleName();


    public BackupRestoreService() {
        super("LeoTweaksWorkingThread");
    }
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (intent != null) {
            switch (intent.getAction()) {
                case SERVICE_INTENT_ACTION_BACKUP:
                    handleMainFolder();
                    iteratePrefsAndBackup();
                    break;
                case SERVICE_INTENT_ACTION_RESTORE:
                    String filePath = intent.getStringExtra(BACKUP_FILE_PATH_EXTRA_KEY);
    ...
    private void iteratePrefsAndBackup() {
        File prefsFolder = new File(SHARED_PREFS_FOLDER_PATH);
    ...
    private void makeBackup(String backupString) {
        ...
        File newBackupFile = new File(BACKUP_FOLDER_PATH + File.separator + backupFileName);
        ...
        try {
            ...
            intent.putExtra(BACKUP_FILE_PATH_EXTRA_KEY, String.format(Locale.getDefault(), getString(R.string.backup_complete_toast), newBackupFile.getAbsoluteFile()));
    ...
    private void handleMainFolder() {
        File mainFolder = new File(BACKUP_FOLDER_PATH);
    ...
    private class UiThreadRunnable implements Runnable {
        ...
        @Override
        public void run() {
            Toast.makeText(LeoAmberApplication.getContext(), mMessage, Toast.LENGTH_SHORT).show();
        }
    }

    private class LocalReceiver extends BroadcastReceiver {
        static final String BACKUP_COMPLETE_ACTION = "com.leo.salt.action.BACKUP_COMPLETE";
        static final String RESTORE_COMPLETE_ACTION = "com.leo.salt.action.RESTORE_COMPLETE";
