...
public class FloatingService extends TileService {
    ...
    public void onClick() {
        ...
        if (toggleState == STATE_ON) {
            ...
            icon =  Icon.createWithResource(getApplicationContext(), R.drawable.ic_ball);
    ...
    public static  void  isOmipopModel(Context ctx,int b) {
     int style;
        style = Settings.System.getInt(ctx.getContentResolver(), "leo_tweaks_ball_display", b) ^ 1;
        Settings.System.putInt(ctx.getContentResolver(), "leo_tweaks_ball_display",  style);
        if ( style== 0){
            Toast.makeText(ctx, "☹ "+"禁用悬浮球", 0).show();

        }else if( style== 1){
            Toast.makeText(ctx, "☺ "+"启用悬浮球"+mNewline+"这是一个不错的全面屏工具哦", 0).show();
        }
    }
    @Override
