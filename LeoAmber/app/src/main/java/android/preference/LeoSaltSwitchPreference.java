...
public class LeoSaltSwitchPreference extends SwitchPreference implements LeoSaltPreferenceScreen.CustomDependencyListener {
    ...
    public LeoSaltSwitchPreference(Context context, AttributeSet attrs) {
        super(context,attrs);
        ini_preference(context, attrs);
    }

    public LeoSaltSwitchPreference(Context context) {
        super(context);
    }

    public LeoSaltSwitchPreference(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void ini_preference(Context context, AttributeSet attrs){
        ...
        TypedArray ta;
        if(Common.mContextWrapper!=null) ta = Common.mContextWrapper.obtainStyledAttributes(attrs, R.styleable.LeoSaltSwitchPreference);
        else ta = context.obtainStyledAttributes(attrs, R.styleable.LeoSaltSwitchPreference);
        //TypedArray ta = getContext().obtainStyledAttributes(att, R.styleable.LeoSaltSwitchPreference);
        if(ta.hasValue(R.styleable.LeoSaltSwitchPreference_switchColor)) {
            try {
                mColor = ta.getColor(R.styleable.LeoSaltSwitchPreference_switchColor, 0);
            } catch (Exception e) {
            }
        }
        ta.recycle();
