...
import org.leo.tweaks.salt.act.LeoSaltImagePicker;
import org.leo.tweaks.salt.utils.LeoSaltImageHelper;

import java.io.File;



public class LeoSaltPickImage extends LeoSaltBasePreference{
    ...
    public LeoSaltPickImage(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAttributes(context, attrs);
    }


    public LeoSaltPickImage(Context context, AttributeSet attrs, int defStyleAttr){
    ...
    private void initAttributes(Context context, AttributeSet attrs){
        ...
        mSizeX=ta.getInt(R.styleable.LeoSaltPreferences_sizeX,0);
        mSizeY=ta.getInt(R.styleable.LeoSaltPreferences_sizeY,0);
        mCircular=ta.getBoolean(R.styleable.LeoSaltPreferences_circularImage,false);
        ta.recycle();
    ...
    public void showDialog(){
        ...
        LeoSaltPreferenceScreen chl = (LeoSaltPreferenceScreen) getOnPreferenceChangeListener();
        if(chl!=null){

            if(mJustUri) {
                Intent intent = new Intent(chl.getActivity(), LeoSaltImagePicker.class);
                intent.putExtra(Common.TAG_DEST_FRAGMENT_NAME_EXTRA_KEY,myPrefAttrsInfo.getMyKey());
                intent.putExtra(LeoSaltImagePicker.S_URI_MODE,true);
                chl.startImagePicker(intent, Common.REQ_CODE_GALLERY_IMAGE_PICKER_JUST_URI);
            }
            else {
                Intent intent = new Intent(chl.getActivity(), LeoSaltImagePicker.class);
                intent.putExtra(Common.TAG_DEST_FRAGMENT_NAME_EXTRA_KEY,myPrefAttrsInfo.getMyKey());
                intent = LeoSaltImageHelper.intent_avatar_img(intent, mSizeX, mSizeY,mCircular);
                String output_file_name = Common.IconsDir + File.separator + String.valueOf(System.currentTimeMillis()+".jpg");
                intent.putExtra(LeoSaltImagePicker.S_OUTPUT_FILE_NAME,output_file_name);
    ...
    public void deletCurrentImgFile(){
          ...
          if(mStringValue!=null){
              detele_current_img = mStringValue.contains( getContext().getString(R.string.LeoSalts_data_base_folder)+File.separator+getContext().getString(R.string.LeoSalts_data_icons_subfolder) );
          }
    ...
    private class ImageLoader extends AsyncTask<Void, Void,Void> {
        ...
        protected Void doInBackground(Void...params) {
            if(mStringValue!=null && !mStringValue.isEmpty()){
                    mWidgetIcon = LeoSaltImageHelper.get_scaled_drawable_from_uri_string_for_square_container(getContext(),mStringValue,mIconSize);
