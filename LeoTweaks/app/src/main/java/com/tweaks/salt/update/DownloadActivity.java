...
import com.tweaks.salt.utils.root.DownloadService;
...
import static com.tweaks.salt.utils.Constants.LeoOTAHTTP;
import static com.tweaks.salt.utils.Constants.OS_VERSION;
import static com.tweaks.salt.utils.Constants.OtaUrl;
import static com.tweaks.salt.utils.NetUtils.hasNetWork;
  ...
  //  String currentTag=null;
    ...
    class OTAanalysis extends BaseAdapter {
        ...
        public View getView(int i, View view, ViewGroup viewGroup) {
            ...
            itemTag.ROMVersions.setText("当前版本: "+AndroidUtils.getROMVersion());
            itemTag.ROMDate.setText("发布日期: "+lists.get(i).getContent());
            itemTag.OtaVersions.setText("OTA版本: "+lists.get(i).getTime());
            itemTag.OtaDate.setText("发布日期: "+lists.get(i).getOTA());
            itemTag.OtaStopDate.setText("截止日期: "+lists.get(i).getOtaStopDate());
            itemTag.OtaPush.setText("是否推送: "+lists.get(i).getOtaPush());
            itemTag.ChangeLog.setText("更新日志: "+lists.get(i).getChangeLog());
            return view;
        }
    }
...
//                    }
    ...
    public void createSDCardDirimg(){
        ...
        if(Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())){
            ...
            String path=sdcardDir.getPath()+"/LeoTweaks/ota";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("OTA更新");
    ...
    private void checkNetworkInf0() {
       ...
       if (hasNetWork(getApplicationContext()) == 0) {
           AlertDialog dialog = new AlertDialog.Builder(mContext)
                   .setTitle("\uD83D\uDE13\uD83D\uDE13\uD83D\uDE13\uD83D\uDE13")
                   .setMessage("当前未有网络连接,无法使用OTA服务")//设置对话框的内容
                   //设置对话框的按钮
           ...
           AlertDialog dialog = new AlertDialog.Builder(this)
                   .setTitle("\uD83D\uDE31\uD83D\uDE31\uD83D\uDE31\uD83D\uDE31")
                   .setMessage("我们检测到你当前是移动数据状态,由于在线更新使用流量数据较大,我们建议你切换到无线网络!在使用OTA服务!")//设置对话框的内容
                   //设置对话框的按钮
                   .setNeutralButton("下次", new DialogInterface.OnClickListener() {
                       @Override
                       public void onClick(DialogInterface dialog, int which) {
                           onBackPressed();
                   ...
                   .setPositiveButton("就是任性使用使用数据", new DialogInterface.OnClickListener() {
                       @Override
                       public void onClick(DialogInterface dialog, int which) {
                           checkOTAInfo(true);
                           dialog.dismiss();
    ...
    private void checkOTAInfo(boolean Network){
        if(Network){
            ...
            progressDialog.setMessage("正在获取更新数据......");
            new MyTask().execute();
        }
    ...
    public  void  OTA(final Activity context) {
        if(AndroidUtils.getROMVersion().equals(OS_VERSION)) {
            downloadBinder.startDownload(LeoOTAHTTP);
        } else {
            Toast.makeText(DownloadActivity.this,"别点了,没可用更新",Toast.LENGTH_SHORT).show();
        }
    }
