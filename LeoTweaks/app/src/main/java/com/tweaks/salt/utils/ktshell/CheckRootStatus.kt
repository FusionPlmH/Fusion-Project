...
import com.tweaks.salt.kt.ProgressBarDialog
    ...
    companion object {
        ...
        fun grantPermission(context: Context) {
            ...
            if (!checkPermission(context, Manifest.permission.READ_EXTERNAL_STORAGE)) {
                cmds.append("pm grant com.tweaks.salt android.permission.READ_EXTERNAL_STORAGE;\n")
            }
            if (!checkPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                cmds.append("pm grant com.tweaks.salt android.permission.WRITE_EXTERNAL_STORAGE;\n")
            }
            if (!checkPermission(context, Manifest.permission.CHANGE_CONFIGURATION)) {
                cmds.append("pm grant com.tweaks.salt android.permission.CHANGE_CONFIGURATION;\n")
            }
            if (!checkPermission(context, Manifest.permission.WRITE_SECURE_SETTINGS)) {
                cmds.append("pm grant com.tweaks.salt android.permission.WRITE_SECURE_SETTINGS;\n")
            }
        ...
        public fun isTmpfs(dir: String): Boolean {
            return executeCommandWithOutput(false, " df | grep tmpfs | grep \"$dir\"").trim().length > 0
        }
    }
}
