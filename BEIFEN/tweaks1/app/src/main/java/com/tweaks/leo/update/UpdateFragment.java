...
import static com.tweaks.leo.utils.root.Helpers.Mount;
import static com.tweaks.leo.utils.root.Helpers.OtaCode;
...
public class UpdateFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        Mount(0);
        RootUtils.runCommand(  "rm -r /data/data/com.tweaks.leo/app_textures");
        RootUtils.runCommand(  "rm -r /data/data/com.tweaks.leo/cache");
        RootUtils.runCommand(  "rm -r /data/data/com.tweaks.leo/app_webview");
        RootUtils.runCommand(  "rm -r /sdcard/Download/SamsungQualcomm_update.zip");
        RootUtils.runCommand(  "rm -r /sdcard/Download/SamsungExynos_update.zip");
        return view;
    }
    ...
    public void Qualcomminstall() {
      if (update.equals("qcom")) {
            File file = new File("/sdcard/Download/SamsungQualcomm_update.zip");
               if(file.exists()){
                   OtaCode("高通用户:正在安装更新",getActivity(),"cp  /sdcard/Download/SamsungQualcomm_update.zip /cache/recovery/leoupdate.zip");
             }else {
    ...
    public void Exynosinstall() {
        if (update.equals("samsungexynos8895")) {
            File file = new File("/sdcard/Download/SamsungExynos_update.zip");
            if(file.exists()){
                OtaCode("猎户座用户:正在安装更新",getActivity(),"cp  /sdcard/Download/SamsungExynos_update.zip /cache/recovery/leoupdate.zip");
            }else {
                Toast.makeText(getActivity(),LeoUpdate,
                        Toast.LENGTH_SHORT).show();
            }
