...
import com.tweaks.leo.utils.root.SuTask;
import com.tweaks.leo.widget.EulaDialog;

import static com.tweaks.leo.utils.AndroidUtils.getCustomDateSystemVersion;
import static com.tweaks.leo.utils.AndroidUtils.gety_idVersion;


public abstract class BaseActivity extends AppCompatActivity implements SuTask.OnSuCompletedListener {
...
//      recreate();
    ...
    public void onTaskCompleted(boolean isGranted) {
        ...
        if (!isGranted && isFirstLaunch) {
            mFragmentManager.beginTransaction().add(AlertDialogFragment.newInstance(Constants.NO_SU_DIALOG_REQUEST_CODE), "no_su_dialog_fragment").commit();
        }
        if (isFirstLaunch) {
